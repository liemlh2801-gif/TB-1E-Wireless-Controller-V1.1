...
function updateOnHoldUi() {
  ...
  updateOnHoldBeep();
  if (connected) {
    requestAnimationFrame(fitDeviceStatusText);
  }
}
...
function fitDeviceStatusText() {
  if (!els.deviceStatus || els.deviceStatus.hidden) {
    return;
  }

  const containerWidth = els.deviceStatus.clientWidth;
  if (containerWidth <= 0) {
    return;
  }
  ...
  const modeLineWidth = containerWidth + PANEL_LAYOUT.modeLineOffsetPx;
  shrinkToFit(els.modeText, modeLineWidth, 26);
